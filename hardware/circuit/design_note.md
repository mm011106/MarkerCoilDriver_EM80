...
5. リモコン回路 (回路図 Sheet 5/5)<br></br>
    ...
    - __光ファイバトランシーバ：__ </br>
        ...
        レシーバはマーカコイルドライバ本体から80Hzモード時のフレーム同期信号を受け取ります。このリモコン回路ではその信号をMEG本体に送信（U35経由）と同時に、動作表示（電源インジケータの点滅）を行っています(U37A)。

    - __操作スイッチ：__ </br>
        操作スイッチおよびMEGからの信号をマーカコイルドライバ本体に伝達するための回路（U36A,B）です。スイッチの操作あるいはMEGからのマーカコイル動作On,Offの指令をパルス幅に変換します。On指令の場合75msのパルス、Offの場合25msと設定してあります。このパルスを光ファイバトランスミッタに入力し本体へ送ります。</br>
        リモコン回路内にはマーカコイルドライバが動作しているかどうかのステータスは保持されていません。

    - __電源：__ </br>
        リモコン回路は光ファイバやフォトカプラで電気的に独立しているので、動作に電源が必要となります。USB-Cの端子から電源だけを利用する形でUSBによる電源供給としています。この設計の理由は、ケーブルの入手性が高いことや、給電する側の選択肢が広くユーザの状況によって選択が可能と言う点です。消費電力はわずか（100mA以下）なので、USBハブからの給電でも駆動できます。</br>
