...
4. Sequencer ~ CPLD周辺回路 (回路図 Sheet 4/5)<br></br>
    ...
    - __制御信号：__ </br>
        全体の動作を決める信号は、POWER_ON, RMT_80/RETHM の2つになります。POWER_ONは全体の動作を開始するための信号で、RMT_80/RETHMは80HzモードとReTHMモードの切り替えの信号です。これらの信号はリモコンインターフェイス回路のステートマシンで作られます。
...
5. リモコン回路 (回路図 Sheet 5/5)<br></br>
    マーカコイルドライバ本体とプラスティック光ファイバで接続されるリモコンの回路図です。</br>
    操作スイッチでの操作とMEG本体からの信号のどちらからでもマーカコイルドライバが操作可能です。MEG本体には80Hzモード時のフレーム同期信号を出力します。MEG本体とはフォトカプラで絶縁されています。
    - __光ファイバトランシーバ：__ </br>
        マーカコイルドライバ本体と通信するための光ファイバドランシーバがU31,U34です。送信側のLED駆動電流は8mAに設定してあります。これは、U31:[HFBR-1521Z](https://docs.broadcom.com/doc/AV02-1501EN)の規定駆動電流（5Mbpsのデータレートを10mのファイバで保証する場合、40~50mA程度）よりもかなり低い電流です。これはマーカコイルドライバ本体の駆動電流にあわせて設定しています。</br>
        このリモコンはSQUIDセンサから離れてMSRの外に設置されますので、規定の動作電流でもいいかもしれません。ただ、実際の駆動周波数帯域は20Hz程度、高調波を加味しても200Hzぐらいですので低電流の駆動で問題ないと考えています。</br>
        駆動はバスバッファIC：74HC540のゲートを並列にして使用しています。駆動のアンバランスによって出力同士で電流のやり取りが無いように各ゲートに抵抗を入れてあります。駆動電流から考えるとかなりオーバースペックの駆動回路と言えると思います。74HC540の最大出力電流は1ICあたり35mA、ゲートあたり6mAとなっているので今回の設定では２〜３ゲートの並列でもいいかもしれません。あるいはMOS-FETなどを使えば簡便に駆動できると思います。</br>
        レシーバはマーカコイルドライバ本体から80Hzモード時のフレーム同期信号を受け取ります。このリモコン回路ではその信号をMEGに送信（U35経由）と同時に、動作表示（電源インジケータの点滅）を行っています(U37A)。

    - __操作スイッチ：__ </br>
        操作スイッチおよびMEGからの信号をマーカコイルドライバ本体に伝達するための回路（U36A,B）です。スイッチの操作あるいはMEGからのマーカコイル動作On,Offの指令をパルス幅に変換します。On指令の場合75msのパルス、Offの場合25msと設定してあります。このパルスを光ファイバトランスミッタに入力しマーカコイルドライバ本体へ送ります。</br>
        リモコン回路内にはマーカコイルドライバが動作しているかどうかのステータスは保持されていません。

    - __電源：__ </br>
        リモコン回路は光ファイバやフォトカプラにより電気的に独立しているので、動作に電源が必要となります。USB-microの端子から電源だけを利用する形でUSBによる電源供給としています。この設計の理由は、ケーブルの入手性が良いこと、給電する側の選択肢が広くユーザの状況によって選択が可能と言う点です。消費電力はわずか（100mA以下）なので、USBハブからの給電でも駆動できます。</br>
        USB-microのコネクタはだいぶ時代遅れになっているので、設計し直す時にはUSB-Cにすることをおすすめします。</br>
