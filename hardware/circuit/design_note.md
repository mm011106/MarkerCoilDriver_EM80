...
4. Sequencer ~ CPLD周辺回路 (回路図 Sheet 4/5)<br></br>
    80Hzモードでのコイルの切り替えタイミングや、電流設定、同期信号の出力、ReTHMモード・80Hzモードの切り替えなどを行うCPLD周りの回路です。CPLDに書き込まれている動作記述のHDLは別途解説があります。</br>
    - __スイッチ入力：__ </br>
    ...
    - 信号名のある端子の利用状況
        ...
        | 名前 | 解説 | 利用状況 |
        ----|----|----
        | RETHM_F4 | ReTHM発振器同期信号（入力） | 未使用|
        | RETHM_F3 | ReTHM発振器同期信号（入力）| 未使用|
        | RETHM_F2 | ReTHM発振器同期信号（入力） | 未使用|
        | RETHM_F1 | ReTHM発振器同期信号（入力） | 未使用|
        | RETHM_F0 | ReTHM発振器同期信号（入力） | 未使用|
        | !RETHM_OE |ReTHM発振器出力イネーブル | 利用|
        | !LOW_BAT |バッテリ電圧低下 | 未使用|
        | CH_2 | 80Hz MUXチャンネル設定 |利用|
        | CH_1 | 80Hz MUXチャンネル設定 |利用|
        | CH_0 | 80Hz MUXチャンネル設定 |利用|
        | 80_OE | 80Hz MUX出力イネーブル |利用|
        | SYNC_80 | 80Hz 同期信号（CPLD内部動作クロック） |利用|
        | AMP_0| 電流設定（電圧制御設定） |利用|
        | AMP_1 | 電流設定（電圧制御設定） |利用|
        | ZOUT | 電流設定（出力インピーダンス設定） |利用|
        | COMP_80 | 80Hz同期信号作成用コンパレータOn/Off |利用(Hi固定)|
        | RETHM_POWER | RETHM発振器電源制御 |利用|
        | SW_80Hz/!RETHM | 80Hz/ReTHM切り替え |未使用|
        | SW_OPTION | 予備のスイッチ入力 |未使用|
        | SW_Z0_10K/!100K |電流設定（インピーダンス切替） |利用|
        | SW_AMP_X10/!X1 | 電流設定（振幅切替） |利用|
        | ALM_LOW_BAT | LowBatteryアラーム出力 |未使用|
        | RMT_POWER | リモコンからの直接制御 |未使用|
        | RMT_80/RETHM | 80Hzモード・ReTHMモード切替入力 |利用|
        | RMT_Z0_10K/100K | リモコンからの直接制御 |未使用|
        | RMT_AMP_X10/X1 | リモコンからの直接制御 |未使用|
        | POWER_ON | 動作開始入力 |利用|
        | FRAME_SYNC | 80Hz時の1シーケンス毎同期信号出力（リモコン用） |利用|
        | RETHM_SYNC0 | ReTHM発振器同期信号（出力） |利用|
        | RETHM_SYNC1 | ReTHM発振器同期信号（出力） |利用|
        | RETHM_SYNC2 | ReTHM発振器同期信号（出力） |利用|
        | RETHM_SYNC3 | ReTHM発振器同期信号（出力） |利用|
        | RETHM_SYNC4 | ReTHM発振器同期信号（出力） |利用|
