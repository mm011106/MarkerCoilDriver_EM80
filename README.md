...
コイル駆動回路とリモコンの間の信号はMEG装置へのノイズ流入を防ぐため、プラスティック光ファイバーにより伝送されます。リモコン→コイル駆動回路への信号はパルス幅変調による1bitの情報で、EM-80Dの動作モードの変更を行います。コイル駆動回路→リモコンへの信号はコイル駆動のタイミング同期信号となっています。
